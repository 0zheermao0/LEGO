...
# --- LLM Specific ---
parser.add_argument("--llm", type=str, default='qwen2.5:7b') # Updated Default
...
if not config.ablate_moe and not config.ablate_llm_select:
    ...
    if not expert_selections:
        ...
        # --- Call Ollama for each prompt ---
        print(f"Calling Ollama ({config.llm}) for {len(prompts)} prompts...")
        for node_id, prompt in enumerate(prompts):
            ...
            try:
                response = ollama.generate(
                    ...
                    prompt=prompt,
                    format='json',
                    options={'temperature': 0, 'num_ctx': 4096, 'num_predict': 1024} # Reduced context/predict
                )
                ...
                print(f"Node {node_id}: Ollama API error: {e_ollama}. Check Ollama server connection/status. Defaulting to random.")
                expert_selections[node_id] = np.random.randint(0, config.expert_num)
                # Optional: Add a delay or break if Ollama errors persist
                # time.sleep(2)
