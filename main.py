...
parser.add_argument("--drop_out", type=float, default=4e-1)

parser.add_argument("--perturb", type=bool, default=False)
parser.add_argument("--perturb_value", type=float, default=0)
...
# llm
#max_lengt llm_name = '/home/Joey/exp/llm_models/gemma-3-4b/'
# llm = AutoModelForCausalLM.from_pretrained(llm_name, torch_dtype=torch.bfloat16).to(device)
# llm.eval()
...
encoder = MoE(input_size=source_data.num_features, output_size=encoder_dim, num_experts=args.expert_num, k=1
          , gnn_type='ppmi', num_hops=num_hops_config).to(device)
...
def calculate_expert_uncertainty(experts_outputs, num_classes, cls_model):
    """
    ...
    # 选择不确定性最高的k个节点
    k = min(5, uncertainty.size(0))  # 选择前100个不确定性最高的节点，或者节点总数如果少于100
...
def train(epoch):
    ...
    # 计算专家间不确定性并生成mask
    uncertainty_mask = calculate_expert_uncertainty(experts_outputs, source_data.num_classes, cls_model)
    ...
    with torch.no_grad():
        ...
        if not uncertainty_node_indices:
            ...
            prompts = [
                ...
                and 3 GNN experts: (0:1-hop, 1:2-hop, 2:3-hop), give out your choice on expert directly for node {node_id}.
                Please note:
            ...
            for idx, node_id in enumerate(uncertainty_node_indices):
                ...
                try:
                    response = ollama.generate(
                        model='gemma3:4b-it-fp16',  # 替换为你实际的ollama模型名
                        prompt=prompt,
                        format='json'
                    )
                    ...
                    try:
                        ...
                        expert_selections[node_id] = parsed_json.get("expert", 0)
                    except json.JSONDecodeError as e:
                        print(f"解析节点 {node_id} 的LLM专家选择建议失败: {json_output_str}")
                        expert_selections[node_id] = 0  # 默认值
    ...
    # 获取MoE的专家分配
    moe_expert_indices, moe_expert_probs = encoder.get_node_expert_assignment(source_data.x)
    ...
    if expert_selections:
        ...
        # 计算交叉熵损失
        moe_softmax = torch.softmax(moe_expert_dist, dim=-1)
        select_loss = -torch.sum(llm_expert_dist * torch.log(moe_softmax + 1e-9)) / len(uncertainty_node_indices)
        # print(f"Entropy Loss between MoE and LLM selections: {select_loss.item()}")
